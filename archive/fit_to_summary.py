...
def main():
    ...
    for f in args.fit_files:
        if args.multisport:
            ...
            results, df_sets = summarize_fit_sessions(
                f, ftp=args.ftp, hr_rest=args.hrrest, hr_max=args.hrmax, tz_name=args.tz
            )
            ...
            for result in results:
                if result:
                    ...
                    if session_key not in processed_sessions:
                        ...
                        print(
                            f"Skipping duplicate session: {result['sport']} at {result['start_time']}"
                        )
        else:
            # Original behavior
            s, df_sets = summarize_fit_original(
                f, ftp=args.ftp, hr_rest=args.hrrest, hr_max=args.hrmax, tz_name=args.tz
            )
            if s:
                rows.append(s)
