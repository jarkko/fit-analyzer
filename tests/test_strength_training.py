...
class TestStrengthTrainingExtraction:
    ...
    def test_summarize_fit_original_returns_strength_sets(self, strength_fit_file):
    ...
    def test_strength_sets_have_active_and_rest_types(self, strength_fit_file):
        """Test that strength sets include both active and rest set types."""
        _, df_sets = summarize_fit_original(strength_fit_file, ftp=300)

        set_types = df_sets["set_type"].unique()
        assert "active" in set_types
        assert "rest" in set_types

    def test_active_sets_have_repetitions(self, strength_fit_file):
...
class TestStrengthTrainingAggregation:
    ...
    def test_aggregate_strength_sets_returns_dataframe(self, multiple_strength_files):
        ...
        df_summary = _aggregate_strength_sets(
            multiple_strength_files, ftp=300, hr_rest=50, hr_max=190, tz_name="Europe/Helsinki"
        )
    ...
    def test_aggregate_extracts_sport_information(self, strength_fit_file):
        ...
        df_summary = _aggregate_strength_sets(
            [strength_fit_file], ftp=300, hr_rest=50, hr_max=190, tz_name="Europe/Helsinki"
        )

        # Sport should not be "unknown" for valid strength training files
        assert df_summary["sport"].iloc[0] != "unknown"
        assert df_summary["sport"].iloc[0] == "training"
        assert df_summary["sub_sport"].iloc[0] == "strength_training"
    ...
    def test_aggregate_includes_activity_id(self, multiple_strength_files):
        ...
        assert unique_activities == len(multiple_strength_files)
    ...
    def test_aggregate_sorts_by_date_and_timestamp(self, multiple_strength_files):
    ...
    def test_aggregate_handles_empty_file_list(self):
        ...
        df_summary = _aggregate_strength_sets(
            [], ftp=300, hr_rest=50, hr_max=190, tz_name="Europe/Helsinki"
        )

        assert df_summary is None

    def test_aggregate_counts_sets_correctly(self, multiple_strength_files):
...
class TestExerciseNames:
    ...
    def test_sets_have_exercise_name_column(self, strength_fit_file):
        """Test that strength sets include exercise_name column."""
        _, df_sets = summarize_fit_original(strength_fit_file, ftp=300)

        assert "exercise_name" in df_sets.columns

    def test_exercise_names_are_human_readable(self, strength_fit_file):
    ...
    def test_aggregate_includes_exercise_name(self, multiple_strength_files):
    ...
    def test_exercise_names_vary_across_sets(self, strength_fit_file):
