...
def sample_multisport_metadata():
    ...
    return {
        ...
        "metadataDTO": {"childIds": [20744294782, 20744294788], "isOriginal": True},
    }
...
class TestFetchExerciseSetsFromAPI:
    ...
    def test_fetch_exercise_sets_for_regular_activity(self, mock_garth, sample_api_exercise_sets):
        ...
        mock_garth.connectapi.assert_any_call(
            f"/activity-service/activity/{activity_id}/exerciseSets"
        )

    def test_fetch_exercise_sets_for_multisport_activity(
        self, mock_garth, sample_multisport_metadata, sample_api_exercise_sets
    ):
...
class TestMergeAPIExerciseNames:
    ...
    def test_merge_replaces_exercise_names_by_message_index(self):
        ...
        assert (
            result_df.loc[result_df["message_index"] == 80, "exercise_name"].iloc[0]
            == "Weighted Ghd Back Extensions"
        )
        assert (
            result_df.loc[result_df["message_index"] == 48, "exercise_name"].iloc[0]
            == "Dumbbell Kickback"
        )
        # Should keep original for non-matching
        assert (
            result_df.loc[result_df["message_index"] == 35, "exercise_name"].iloc[0]
            == "Barbell Deadlift"
        )
    ...
    def test_merge_handles_empty_exercise_sets(self):
        ...
        fit_df = pd.DataFrame(
            {"message_index": [1], "exercise_name": ["Squat"], "repetitions": [10.0]}
        )
    ...
    def test_merge_converts_api_name_format(self):
        ...
        fit_df = pd.DataFrame(
            {"message_index": [10], "exercise_name": ["Unknown"], "repetitions": [8.0]}
        )
    ...
    def test_merge_handles_multiple_exercises_in_set(self):
        ...
        fit_df = pd.DataFrame(
            {"message_index": [5], "exercise_name": ["Unknown"], "repetitions": [12.0]}
        )
